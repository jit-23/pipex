...
/*   Updated: 2023/12/28 03:37:15 by fde-jesu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	find_path(char ** env, t_pipex *pipex)
{
	...
	while(env[++i])
		if (ft_strnstr(env[i], search, sizeof(search)) != NULL)
			pipex->path = ft_substr(env[i], ft_strlen("PATH="), ft_strlen(env[i]));
	pipex->path_arr1 = organize_env(pipex->path_arr1,pipex->path, pipex->cmd1);
	pipex->path_arr2 = organize_env(pipex->path_arr2,pipex->path, pipex->cmd2);
